<?php
	...
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtPassword'])=="") {
			$message[] = "<b>txtPassword</b> tidak boleh kosong !";		
		}
		...
		if(count($message)==0){
			$sqlSave	= "UPDATE it_note SET nama='$txtNama',
	...
	$sqlShow			= "SELECT * FROM it_note WHERE id_it_note='$KodeEdit'";
	$qryShow 			= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data department salah : ".mysqli_errors());
	$dataShow			= mysqli_fetch_array($qryShow);
	
	$dataKode			= $dataShow['id_it_note'];
	$dataNama			= isset($_POST['txtNama']) ? $_POST['txtNama'] : $dataShow['nama'];
	$dataUsername		= isset($_POST['txtUsername']) ? $_POST['txtUsername'] : $dataShow['username'];
	$dataPassword		= isset($_POST['txtPassword']) ? $_POST['txtPassword'] : $dataShow['password'];
	$dataIpAddress		= isset($_POST['txtIpAddress']) ? $_POST['txtIpAddress'] : $dataShow['ip_address'];
