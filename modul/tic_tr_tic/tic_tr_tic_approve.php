 ...
 <?php
	...
	if(isset($_POST['btnApprove'])){
		...
		foreach ($txtID as $id_key) {
			...
			$hapus=mysqli_query($koneksidb, "UPDATE tic_tr_ticket SET tic_tr_ticket_app='Y',
																		tic_tr_ticket_tgl_approve='".date('Y-m-d H:i:s')."'
																WHERE tic_tr_ticket_id='$id_key'") 
				or die ("Gagal kosongkan tmp".mysqli_error());
				
			if($hapus){
				$_SESSION['info'] = 'success';
				$_SESSION['pesan'] = 'Data ticket berhasil approve';
				echo '<script>window.location="?page='.base64_encode(ticapprovaltic).'"</script>';
			}	
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
	<div class="portlet box <?php echo $dataPanel; ?>">
		...
		<div class="caption"><span class="caption-subject uppercase bold">Data Approval Ticket</span></div>
			<div class="actions">
				<button class="btn <?php echo $dataPanel; ?> active" name="btnApprove" type="submit" onclick="return confirm('Anda yakin ingin approve data penting ini !!')"><i class="icon-check"></i> APPROVE DATA</button>
				<button class="btn <?php echo $dataPanel; ?> active" name="btnHapus" type="submit" onclick="return confirm('Anda yakin ingin membatalkan data penting ini !!')"><i class="icon-close"></i> CANCEL DATA</button>
			</div>
		</div>
				...
				<tbody>
                    <?php
						$dataSql = "SELECT * FROM tic_tr_ticket a
									...
									INNER JOIN tic_ms_lvl c ON a.tic_ms_lvl_id=c.tic_ms_lvl_id
									INNER JOIN tic_ms_modul d ON a.tic_ms_modul_id=d.tic_ms_modul_id
									WHERE a.tic_tr_ticket_app='N' AND a.tic_tr_ticket_sts='N'
