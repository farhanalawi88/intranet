<?php
	...
	if(isset($_POST['btnHapus'])){
		...
		foreach ($txtID as $id_key) {
			...
			$hapus=mysqli_query($koneksidb,"DELETE FROM ptkp_ms_kategori WHERE ptkp_ms_kategori_id='$id_key'") 
				or die ("Gagal kosongkan tmp".mysqli_errors());
			if($hapus){
				$_SESSION['info'] = 'success';
	            $_SESSION['pesan'] = 'Data kategori temuan PTKP berhasil dihapus';
           ...
           <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_2">
				<thead>
                    <tr class="active">
                        ...
                        <th width="30%">KATEGORI TEMUAN</th>
				...
				<tbody>
               <?php
						$dataSql = "SELECT * FROM ptkp_ms_kategori a
									INNER JOIN sys_role b ON a.ptkp_ms_kategori_createdby=b.sys_role_id
									ORDER BY a.ptkp_ms_kategori_id DESC";
						$dataQry = mysqli_query($koneksidb, $dataSql);
						$nomor  = 0; 
						while ($data = mysqli_fetch_array($dataQry)) {
						$nomor++;
						$Kode = $data['ptkp_ms_kategori_id'];
						if($data ['ptkp_ms_kategori_sts']=='Y'){
				...
				?>
                    <tr class="odd gradeX">
						...
						<td><?php echo $data['ptkp_ms_kategori_ket']; ?></td>
						<td><?php echo strtoupper($data['sys_role_nama']); ?></td>
						<td><div align="center"><?php echo $dataStatus; ?></div></td>
						<td><div align="center"><a href="?page=<?php echo base64_encode(ptkpedtmskategori) ?>&amp;id=<?php echo base64_encode($Kode); ?>" class="btn btn-xs <?php echo $dataPanel; ?>"><i class="fa fa-pencil"></i></a></div></td>
                    </tr>
                    <?php
