<?php
	...
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){
			$sqlSave	= "UPDATE ptkp_ms_kategori SET ptkp_ms_kategori_nm='$txtNama', 
													ptkp_ms_kategori_ket='$txtKeterangan', 
													ptkp_ms_kategori_sts='$cmbStatus',
													ptkp_ms_kategori_updated='".date('Y-m-d H:i:s')."',
													ptkp_ms_kategori_updatedby='".$_SESSION['sys_role_id']."'
										WHERE ptkp_ms_kategori_id='$txtID'";
			$qrySave	= mysqli_query($koneksidb, $sqlSave) or die ("gagal insert". mysqli_errors());
			if($qrySave){
				$_SESSION['info'] = 'success';
				$_SESSION['pesan'] = 'Data kategori temuan PTKP berhasil diperbaharui';
	...
	$sqlShow			= "SELECT * FROM ptkp_ms_kategori WHERE ptkp_ms_kategori_id='$KodeEdit'";
	$qryShow 			= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data department salah : ".mysqli_errors());
	$dataShow			= mysqli_fetch_array($qryShow);
	
	$dataID				= $dataShow['ptkp_ms_kategori_id'];
	$dataNama			= isset($_POST['txtNama']) ? $_POST['txtNama'] : $dataShow['ptkp_ms_kategori_nm'];
	$dataKeterangan		= isset($_POST['txtKeterangan']) ? $_POST['txtKeterangan'] : $dataShow['ptkp_ms_kategori_ket'];
	$dataStatus			= isset($_POST['cmbStatus']) ? $_POST['cmbStatus'] : $dataShow['ptkp_ms_kategori_sts'];
?>
