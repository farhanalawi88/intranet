<?php
	if(isset($_POST['btnSave'])){
		...
		$sqlCek			= "SELECT COUNT(*) as total FROM tic_ms_lvl WHERE tic_ms_lvl_nm='$txtNama'";
		$qryCek			= mysqli_query($koneksidb, $sqlCek) or die ("Gagal cek value".mysqli_errors()); 
		$qryRow			= mysqli_fetch_array($qryCek);
		if($qryRow['total']>=1){
			$message[] = "Maaf, nama level <b> $txtNama </b> sudah ada, ganti dengan nama lain";
		}
		
		if(count($message)==0){		
			$sqlSave	= "INSERT INTO tic_ms_lvl (tic_ms_lvl_nm,
													tic_ms_lvl_ket,
													tic_ms_lvl_sts,
													tic_ms_lvl_created,
													tic_ms_lvl_createdby) 
											VALUES ('$txtNama',
													'$txtKeterangan',
													'$cmbStatus',
													'".date('Y-m-d H:i:s')."',
			...
			if($qrySave){
				...
				$_SESSION['pesan'] = 'Data level ticket berhasil ditambahkan';
