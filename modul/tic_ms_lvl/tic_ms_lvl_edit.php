<?php
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){		
			$sqlSave	= "UPDATE tic_ms_lvl SET tic_ms_lvl_nm='$txtNama',
													tic_ms_lvl_ket='$txtKeterangan',
													tic_ms_lvl_sts='$cmbStatus',
													tic_ms_lvl_updated='".date('Y-m-d H:i:s')."',
													tic_ms_lvl_updatedby='".$userRow['sys_role_id']."'
												WHERE tic_ms_lvl_id='$txtKode'";
			$qrySave	= mysqli_query($koneksidb, $sqlSave) or die ("gagal insert". mysqli_errors());

			if($qrySave){
				$_SESSION['info'] = 'success';
				$_SESSION['pesan'] = 'Data level ticket berhasil diperbaharui';
	...
	$sqlShow 		= "SELECT * FROM tic_ms_lvl WHERE tic_ms_lvl_id='$KodeEdit'";
	$qryShow 		= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data libur salah : ".mysqli_errors());
	$dataShow 		= mysqli_fetch_array($qryShow);		

	$dataKode 		= $dataShow['tic_ms_lvl_id'];
	$dataNama		= isset($_POST['txtNama']) ? $_POST['txtNama'] : $dataShow['tic_ms_lvl_nm'];
	$dataKeterangan	= isset($_POST['txtKeterangan']) ? $_POST['txtKeterangan'] : $dataShow['tic_ms_lvl_ket'];
	$dataStatus		= isset($_POST['cmbStatus']) ? $_POST['cmbStatus'] : $dataShow['tic_ms_lvl_sts'];
?>
<div class="portlet box <?php echo $dataPanel; ?>">
	<div class="portlet-title">
		<div class="caption"><span class="caption-subject uppercase bold">Form Level Ticket</span></div>
		<div class="tools">
			...
			<div class="form-actions">
			    <div class="row">
		            <div class="col-lg-offset-2 col-lg-10">
		                ...
		                <a href="?page=<?php echo base64_encode(ticdtlvl) ?>" class="btn <?php echo $dataPanel; ?>"><i class="fa fa-undo"></i> Batalkan</a>
		            </div>
			    </div>
			</div>
		</form>
