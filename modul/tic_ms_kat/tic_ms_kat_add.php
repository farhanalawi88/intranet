<?php
	if(isset($_POST['btnSave'])){
		...
		$sqlCek			= "SELECT COUNT(*) as total FROM tic_ms_kat WHERE tic_ms_kat_nm='$txtNama'";
		$qryCek			= mysqli_query($koneksidb, $sqlCek) or die ("Gagal cek value".mysqli_errors()); 
		$qryRow			= mysqli_fetch_array($qryCek);
		if($qryRow['total']>=1){
			$message[] = "Maaf, nama kategori <b> $txtNama </b> sudah ada, ganti dengan nama lain";
		}
		
		if(count($message)==0){		
			$sqlSave	= "INSERT INTO tic_ms_kat (tic_ms_kat_nm,
													tic_ms_kat_ket,
													tic_ms_kat_sts,
													tic_ms_kat_created,
													tic_ms_kat_createdby) 
											VALUES ('$txtNama',
													'$txtKeterangan',
													'$cmbStatus',
													'".date('Y-m-d H:i:s')."',
			...
			if($qrySave){
				...
				$_SESSION['pesan'] = 'Data kategori ticket berhasil ditambahkan';
