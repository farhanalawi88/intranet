<?php
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){		
			$sqlSave	= "UPDATE tic_ms_modul SET tic_ms_modul_nm='$txtNama',
													tic_ms_modul_ket='$txtKeterangan',
													tic_ms_modul_sts='$cmbStatus',
													tic_ms_modul_updated='".date('Y-m-d H:i:s')."',
													tic_ms_modul_updatedby='".$userRow['sys_role_id']."'
												WHERE tic_ms_modul_id='$txtKode'";
			...
			if($qrySave){
				...
				echo '<script>window.location="?page='.base64_encode(ticdtmodul).'"</script>';
			}
			exit;
		}	
	...
	$sqlShow 		= "SELECT * FROM tic_ms_modul WHERE tic_ms_modul_id='$KodeEdit'";
	$qryShow 		= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data libur salah : ".mysqli_errors());
	$dataShow 		= mysqli_fetch_array($qryShow);		

	$dataKode 		= $dataShow['tic_ms_modul_id'];
	$dataNama		= isset($_POST['txtNama']) ? $_POST['txtNama'] : $dataShow['tic_ms_modul_nm'];
	$dataKeterangan	= isset($_POST['txtKeterangan']) ? $_POST['txtKeterangan'] : $dataShow['tic_ms_modul_ket'];
	$dataStatus		= isset($_POST['cmbStatus']) ? $_POST['cmbStatus'] : $dataShow['tic_ms_modul_sts'];
?>
		...
		<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="frmadd" class="form-horizontal form-bordered" autocomplete="off">
			...
			<div class="form-body">
				<div class="form-group">
					<label class="col-md-2 control-label">Nama Modul :</label>
					<div class="col-md-5">
			...
			<div class="form-actions">
			    <div class="row">
		            <div class="col-lg-offset-2 col-lg-10">
		                ...
		                <a href="?page=<?php echo base64_encode(ticdtmodul) ?>" class="btn <?php echo $dataPanel; ?>"><i class="fa fa-undo"></i> Batalkan</a>
		            </div>
			    </div>
			</div>
		</form>
