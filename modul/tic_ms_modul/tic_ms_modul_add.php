<?php
	if(isset($_POST['btnSave'])){
		...
		$sqlCek			= "SELECT COUNT(*) as total FROM tic_ms_modul WHERE tic_ms_modul_nm='$txtNama'";
		$qryCek			= mysqli_query($koneksidb, $sqlCek) or die ("Gagal cek value".mysqli_errors()); 
		$qryRow			= mysqli_fetch_array($qryCek);
		if($qryRow['total']>=1){
			$message[] = "Maaf, nama modul <b> $txtNama </b> sudah ada, ganti dengan nama lain";
		}
		
		if(count($message)==0){		
			$sqlSave	= "INSERT INTO tic_ms_modul (tic_ms_modul_nm,
													tic_ms_modul_ket,
													tic_ms_modul_sts,
													tic_ms_modul_created,
													tic_ms_modul_createdby) 
											VALUES ('$txtNama',
													'$txtKeterangan',
													'$cmbStatus',
													'".date('Y-m-d H:i:s')."',
			...
			if($qrySave){
				...
				$_SESSION['pesan'] = 'Data modul ticket berhasil ditambahkan';
