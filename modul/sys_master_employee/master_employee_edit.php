<?php
	...
	if(isset($_POST['btnSave'])){
		...
		$txtID						= $_POST['txtID'];
		$txtNik						= $_POST['txtNik'];
		$txtNamaKaryawan			= $_POST['txtNama'];
		$txtKeterangan				= $_POST['txtKeterangan'];
		$cmbStatus					= $_POST['cmbStatus'];
		
		if(count($message)==0){
			$sqlSave	= "UPDATE sys_employee SET nik='$txtNik',
												   nama_karyawan='$txtNamaKaryawan', 
												   keterangan='$txtKeterangan', 
												   status='$cmbStatus',
												   updated='".date('Y-m-d H:i:s')."',
												   updatedby='".$_SESSION['sys_role_id']."'
										WHERE id_employee='$txtID'";
			$qrySave	= mysqli_query($koneksidb, $sqlSave) or die ("gagal insert". mysqli_errors());
			if($qrySave){
				$_SESSION['info'] = 'success';
				$_SESSION['pesan'] = 'Data Employee Berhasil Diperbaharui';
	...
	$sqlShow			= "SELECT * FROM sys_employee WHERE id_employee='$KodeEdit'";
	$qryShow 			= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data department salah : ".mysqli_errors());
	$dataShow			= mysqli_fetch_array($qryShow);
	
	$dataID				= $dataShow['id_employee'];
	$dataNik			= isset($_POST['txtNik']) ? $_POST['txtNik'] : $dataShow['nik'];
	$dataNama			= isset($_POST['txtNama']) ? $_POST['txtNama'] : $dataShow['nama_karyawan'];
...
<div class="portlet box <?php echo $dataPanel; ?>">
	<div class="portlet-title">
        <div class="caption">
            <span class="caption-subject uppercase bold">Form Employee</span>
        </div>
        <div class="tools">
        ...
        <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal form-bordered" autocomplete="off">
        	<div class="form-body">
		        ...
		        <div class="form-group">
					...
					<div class="col-lg-4">
						<input type="text" name="txtNik" value="<?php echo $dataNik; ?>" class="form-control" placeholder="Enter Name" onkeyup="javascript:this.value=this.value.toUpperCase();"/>
		             </div>
				</div>
				<div class="form-group">
	    	...
	    	<div class="form-actions">
                <div class="row">
                    <div class="col-md-offset-2 col-md-10">
		                ...
		                <a href="?page=<?php echo base64_encode(masteremployeedata) ?>" class="btn <?php echo $dataPanel; ?>"><i class="fa fa-undo"></i> Batalkan</a>
			        </div>
			    </div>
			</div>
		</form>
