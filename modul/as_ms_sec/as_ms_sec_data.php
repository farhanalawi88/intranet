<?php
	// Menghapus Data
	if(isset($_POST['btnHapus'])){
		$txtID 		= $_POST['txtID'];
		foreach ($txtID as $id_key) {
			
			$sqlShow			= "SELECT * FROM as_sec WHERE as_sec_id='$id_key'";
			$qryShow 			= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data department salah : ".mysqli_errors());
			while ($dataShow	= mysqli_fetch_array($qryShow)) {
				$hapus 			= mysqli_query($koneksidb,"DELETE FROM as_sec WHERE as_sec_id='$id_key'") 
									or die ("Gagal kosongkan tmp".mysqli_errors());
				// HAPUS VIDEO
				$fileVideo 		= $dataShow['as_sec_video'];
				$targetVideo	= "file/".$fileVideo."";
				if(file_exists($targetVideo)){
					unlink($targetVideo);
				}
				// HAPUS FILE
				$filePDF 		= $dataShow['as_sec_pdf'];
				$targetPDF		= "file/".$filePDF."";
				if(file_exists($targetPDF)){
					unlink($targetPDF);
				}
			}
			
			if($hapus){
				$_SESSION['info'] = 'success';
	            $_SESSION['pesan'] = 'Data ateri / education center berhasil dihapus';
	            echo '<script>window.location="?page='.base64_encode(dtsec).'"</script>';
          	}
		
        }
	}	
	
	
?>
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
	<div class="portlet box <?php echo $dataPanel; ?>">
	    <div class="portlet-title">
	        <div class="caption">
	            <span class="caption-subject uppercase">Data Education Center</span>
	        </div>
	        <div class="actions">
				<a href="?page=<?php echo base64_encode(addsec) ?>" class="btn <?php echo $dataPanel; ?> active"><i class="icon-plus"></i> ADD DATA </a>
				<button class="btn <?php echo $dataPanel; ?> active" name="btnHapus" type="submit" onclick="return confirm('Anda yakin ingin menghapus data penting ini !!')"><i class="icon-trash"></i> DELETE DATA</button>
			</div>
		</div>
    	<div class="portlet-body">
           <table class="table table-striped table-hover table-checkable order-column" id="sample_2">
				<thead>
                    <tr class="active">
       	  	  	  	  	<th class="table-checkbox">
                            <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                <input type="checkbox" class="group-checkable" data-set="#sample_2 .checkboxes" />
                                <span></span>
                            </label>
                        </th>
						<th width="5%"><div align="center">NO</div></th>
						<th width="10%">TGL. DIBUAT</th>
						<th width="20%">NAMA BAGIAN</th>
                        <th width="50%">NAMA & JUDUL</th>
				  	  	<th width="10%"><div align="center">STATUS</div></th>
				  	  	<th width="5%"><div align="center">ACTION</div></th>
                    </tr>
				</thead>
				<tbody>
               <?php
						$dataSql = "SELECT * FROM as_sec a
									INNER JOIN sys_bagian b ON a.sys_bagian_id=b.sys_bagian_id
									ORDER BY a.as_sec_id DESC";
						$dataQry = mysqli_query($koneksidb, $dataSql);
						$nomor  = 0; 
						while ($data = mysqli_fetch_array($dataQry)) {
						$nomor++;
						$Kode = $data['as_sec_id'];
						if($data ['as_sec_sts']=='Y'){
							$dataStatus= "<span class='badge badge-success badge-roundless'>ACTIVE</span>";
						}else{
							$dataStatus= "<span class='badge badge-danger badge-roundless'>NON ACTIVE</span>";
						}
				?>
                    <tr class="odd gradeX">
                        <td>
                            <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                <input type="checkbox" class="checkboxes" value="<?php echo $Kode; ?>" name="txtID[<?php echo $Kode; ?>]" />
                                <span></span>
                            </label>
                        </td>
						<td><div align="center"><?php echo $nomor ?></div></td>
						<td><?php echo IndonesiaTgl2($data ['as_sec_tgl']); ?></td>
						<td><?php echo $data['sys_bagian_nm']; ?></td>
						<td><?php echo $data['as_sec_name']; ?></td>
						<td><div align="center"><?php echo $dataStatus; ?></div></td>
						<td><div align="center"><a href="?page=<?php echo base64_encode(edtsec) ?>&amp;id=<?php echo base64_encode($Kode); ?>" class="btn btn-xs <?php echo $dataPanel; ?>"><i class="fa fa-pencil"></i></a></div></td>
                    </tr>
                    <?php
                        
                    }
                    ?>
				</tbody>
            </table>
	    </div>
	</div>
</form>
    	