<?php
	...
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){
			$sqlSave	= "UPDATE sys_org SET sys_org_kd='$txtKode', 
					 							sys_org_nm='$txtNama', 
												sys_org_ket='$txtKeterangan', 
												sys_org_sts='$cmbStatus',
												sys_org_updated='".date('Y-m-d H:i:s')."',
												sys_org_updatedby='".$_SESSION['sys_role_id']."'
										WHERE sys_org_id='$txtID'";
			...
			if($qrySave){
				...
				echo '<script>window.location="?page='.base64_encode(dtorg).'"</script>';
			}
			exit;
		}	
	...
	$sqlShow			= "SELECT * FROM sys_org WHERE sys_org_id='$KodeEdit'";
	$qryShow 			= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data department salah : ".mysqli_errors());
	$dataShow			= mysqli_fetch_array($qryShow);
	
	$dataID				= $dataShow['sys_org_id'];
	$dataKode			= isset($_POST['txtKode']) ? $_POST['txtKode'] : $dataShow['sys_org_kd'];
	$dataNama			= isset($_POST['txtNama']) ? $_POST['txtNama'] : $dataShow['sys_org_nm'];
	$dataKeterangan		= isset($_POST['txtKeterangan']) ? $_POST['txtKeterangan'] : $dataShow['sys_org_ket'];
	$dataStatus			= isset($_POST['cmbStatus']) ? $_POST['cmbStatus'] : $dataShow['sys_org_sts'];
?>
<div class="portlet box <?php echo $dataPanel; ?>">
	<div class="portlet-title">
        <div class="caption">
            <span class="caption-subject uppercase bold">Form Data Organisasi</span>
        </div>
        <div class="tools">
	    	...
	    	<div class="form-actions">
                <div class="row">
                    <div class="col-md-offset-2 col-md-10">
		                ...
		                <a href="?page=<?php echo base64_encode(dtorg) ?>" class="btn <?php echo $dataPanel; ?>"><i class="fa fa-undo"></i> Batalkan</a>
			        </div>
			    </div>
			</div>
		</form>
