<?php
	if(isset($_POST['btnSave'])){
		...
		$sqlCek			= "SELECT COUNT(*) as total FROM load_ms_jns_kend WHERE load_ms_jns_kend_nm='$txtNama'";
		$qryCek			= mysqli_query($koneksidb, $sqlCek) or die ("Gagal cek value".mysqli_errors()); 
		$qryRow			= mysqli_fetch_array($qryCek);
		if($qryRow['total']>=1){
			$message[] = "Maaf, nama jenis kendaraan <b> $txtNama </b> sudah ada, ganti dengan nama lain";
		}
		
		if(count($message)==0){		
			$sqlSave	= "INSERT INTO load_ms_jns_kend (load_ms_jns_kend_nm,
													load_ms_jns_kend_ket,
													load_ms_jns_kend_sts,
													load_ms_jns_kend_created,
													load_ms_jns_kend_createdby) 
											VALUES ('$txtNama',
													'$txtKeterangan',
													'$cmbStatus',
													'".date('Y-m-d H:i:s')."',
			...
			if($qrySave){
				...
				$_SESSION['pesan'] = 'Data jenis kendaraan berhasil ditambahkan';
...
<div class="portlet box <?php echo $dataPanel; ?>">
	<div class="portlet-title">
		<div class="caption"><span class="caption-subject uppercase bold">Form Jenis Kendaran</span></div>
		<div class="tools">
