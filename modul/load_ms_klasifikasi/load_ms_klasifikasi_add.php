<?php
	if(isset($_POST['btnSave'])){
		...
		$sqlCek			= "SELECT COUNT(*) as total FROM load_ms_klasifikasi WHERE load_ms_klasifikasi_nm='$txtNama'";
		$qryCek			= mysqli_query($koneksidb, $sqlCek) or die ("Gagal cek value".mysqli_errors()); 
		$qryRow			= mysqli_fetch_array($qryCek);
		if($qryRow['total']>=1){
			$message[] = "Maaf, nama klasifikasi <b> $txtNama </b> sudah ada, ganti dengan nama lain";
		}
		
		if(count($message)==0){		
			$sqlSave	= "INSERT INTO load_ms_klasifikasi (load_ms_klasifikasi_nm,
													load_ms_klasifikasi_ket,
													load_ms_klasifikasi_sts,
													load_ms_klasifikasi_created,
													load_ms_klasifikasi_createdby) 
											VALUES ('$txtNama',
													'$txtKeterangan',
													'$cmbStatus',
													'".date('Y-m-d H:i:s')."',
			...
			if($qrySave){
				...
				$_SESSION['pesan'] = 'Data klasifikasi muatan berhasil ditambahkan';
