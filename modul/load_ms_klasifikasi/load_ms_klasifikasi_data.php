<?php
	...
	if(isset($_POST['btnHapus'])){
		...
		foreach ($txtID as $id_key) {
			...
			$hapus=mysqli_query($koneksidb, "DELETE FROM load_ms_klasifikasi WHERE load_ms_klasifikasi_id='$id_key'");
				
			if($hapus){
				$_SESSION['info'] = 'success';
				$_SESSION['pesan'] = 'Data petugas berhasil dihapus';
				echo '<script>window.location="?page='.base64_encod(loaddtmsklasifikasi).'"</script>';
			}	
			...
			<table class="table table-hover table-checkable order-column" id="sample_2">
				...
				<tbody>
                    <?php
						...
						$nomor++;
						$Kode = $data['load_ms_klasifikasi_id'];
						if($data['load_ms_klasifikasi_sts']=='Y'){
                   ...
                   <tr>
						...
						<td><?php echo $data ['load_ms_klasifikasi_nm']; ?></td>
						<td><?php echo $data ['load_ms_klasifikasi_ket']; ?></td>
						<td><div align="center"><?php echo $dataStatus; ?></div></td>
                        <td><div align="center"><a href="?page=<?php echo base64_encode(loadedtmsklasifikasi) ?>&amp;id=<?php echo $Kode; ?>" class="btn <?php echo $dataPanel; ?> btn-xs"><i class="icon-book-open"></i></a></div></td>
                    </tr>
                    <?php } ?>
				</tbody>
            </table>
