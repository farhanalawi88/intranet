<?php
	...
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){
			$sqlSave="UPDATE ext_number SET id_employee='$cmbNama',
											sys_bagian_id='$cmbDepartemen',
											line_telepon='$txtLineTelepon',
											updated='".date('Y-m-d H:i:s')."'
									WHERE id_ext_number='$txtKode'";
			$qrySave	= mysqli_query($koneksidb, $sqlSave) or die ("Gagal Update Ext Telepon ". mysqli_errors());
			if($qrySave){
				$_SESSION['info'] = 'success';
				$_SESSION['pesan'] = 'Data Pembuatan Ext Telepon Berhasil Diperbaharui';
	...
	$sqlShow			= "SELECT 
							...
							WHERE a.id_ext_number='$KodeEdit'";
							
	$qryShow 			= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data department salah : ".mysqli_errors());
	$dataShow			= mysqli_fetch_array($qryShow);

	$dataKode 			= $dataShow['id_ext_number'];
	$dataNama			= isset($_POST['cmbNama']) ? $_POST['cmbNama'] : $dataShow['id_employee'];
	$dataDepartemen		= isset($_POST['cmbDepartemen']) ? $_POST['cmbDepartemen'] : $dataShow['sys_bagian_id'];
	$dataLineTelepon	= isset($_POST['txtLineTelepon']) ? $_POST['txtLineTelepon'] : $dataShow['line_telepon'];
        		...
        		<div class="form-group">
					...
					<div class="col-md-4">
						<select name="cmbNama" data-placeholder="Pilih Nama" class="select2 form-control">
							<option value=""></option> 
							<?php
								  $dataSql = "SELECT * FROM sys_employee WHERE status='Y' ORDER BY id_employee ASC";
