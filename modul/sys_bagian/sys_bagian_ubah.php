<?php
	...
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){
			$sqlSave	= "UPDATE sys_bagian SET sys_bagian_kd='$txtKode', 
					 							sys_bagian_nm='$txtNama', 
												sys_bagian_ket='$txtKeterangan', 
												sys_bagian_sts='$cmbStatus',
												sys_bagian_updated='".date('Y-m-d H:i:s')."',
												sys_bagian_updatedby='".$_SESSION['sys_role_id']."'
										WHERE sys_bagian_id='$txtID'";
			$qrySave	= mysqli_query($koneksidb, $sqlSave) or die ("gagal insert". mysqli_errors());
			if($qrySave){
				$_SESSION['info'] = 'success';
				$_SESSION['pesan'] = 'Data bagian berhasil diperbaharui';
	...
	$sqlShow			= "SELECT * FROM sys_bagian WHERE sys_bagian_id='".base64_decode($KodeEdit)."'";
	$qryShow 			= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data department salah : ".mysqli_errors());
	$dataShow			= mysqli_fetch_array($qryShow);
	
	$dataID				= $dataShow['sys_bagian_id'];
	$dataKode			= isset($_POST['txtKode']) ? $_POST['txtKode'] : $dataShow['sys_bagian_kd'];
	$dataNama			= isset($_POST['txtNama']) ? $_POST['txtNama'] : $dataShow['sys_bagian_nm'];
	$dataKeterangan		= isset($_POST['txtKeterangan']) ? $_POST['txtKeterangan'] : $dataShow['sys_bagian_ket'];
	$dataStatus			= isset($_POST['cmbStatus']) ? $_POST['cmbStatus'] : $dataShow['sys_bagian_sts'];
?>
<div class="portlet box <?php echo $dataPanel; ?>">
	<div class="portlet-title">
        <div class="caption">
            <span class="caption-subject uppercase bold">Form Data Bagian</span>
        </div>
        <div class="tools">
	    	...
	    	<div class="form-actions">
                <div class="row">
                    <div class="col-md-offset-2 col-md-10">
		                ...
		                <a href="?page=<?php echo base64_encode(dtbagian) ?>" class="btn <?php echo $dataPanel; ?>"><i class="fa fa-undo"></i> Batalkan</a>
			        </div>
			    </div>
			</div>
		</form>
