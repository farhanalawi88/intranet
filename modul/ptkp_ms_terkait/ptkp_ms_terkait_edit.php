<?php
	...
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){
			$sqlSave	= "UPDATE ptkp_ms_terkait SET ptkp_ms_terkait_nm='$txtNama', 
													ptkp_ms_terkait_ket='$txtKeterangan', 
													ptkp_ms_terkait_sts='$cmbStatus',
													ptkp_ms_terkait_updated='".date('Y-m-d H:i:s')."',
													ptkp_ms_terkait_updatedby='".$_SESSION['sys_role_id']."'
										WHERE ptkp_ms_terkait_id='$txtID'";
			$qrySave	= mysqli_query($koneksidb, $sqlSave) or die ("gagal insert". mysqli_errors());
			if($qrySave){
				$_SESSION['info'] = 'success';
				$_SESSION['pesan'] = 'Data keterkaitan temuan PTKP berhasil diperbaharui';
				echo '<script>window.location="?page='.base64_encode(ptkpdtmsketerkaitan).'"</script>';
			}
			exit;
		}	
	...
	$sqlShow			= "SELECT * FROM ptkp_ms_terkait WHERE ptkp_ms_terkait_id='$KodeEdit'";
	$qryShow 			= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data department salah : ".mysqli_errors());
	$dataShow			= mysqli_fetch_array($qryShow);
	
	$dataID				= $dataShow['ptkp_ms_terkait_id'];
	$dataNama			= isset($_POST['txtNama']) ? $_POST['txtNama'] : $dataShow['ptkp_ms_terkait_nm'];
	$dataKeterangan		= isset($_POST['txtKeterangan']) ? $_POST['txtKeterangan'] : $dataShow['ptkp_ms_terkait_ket'];
	$dataStatus			= isset($_POST['cmbStatus']) ? $_POST['cmbStatus'] : $dataShow['ptkp_ms_terkait_sts'];
?>
<div class="portlet box <?php echo $dataPanel; ?>">
	<div class="portlet-title">
        <div class="caption">
            <span class="caption-subject uppercase bold">Form Data Keterkaitan Temuan PTKP</span>
        </div>
        <div class="tools">
        ...
        <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal form-bordered" autocomplete="off">
        	<div class="form-body">
		        ...
		        <div class="form-group">
					<label class="col-lg-2 control-label">Keterkaitan :</label>
					<div class="col-lg-4">
	    	...
	    	<div class="form-actions">
                <div class="row">
                    <div class="col-md-offset-2 col-md-10">
		                ...
		                <a href="?page=<?php echo base64_encode(ptkpdtmsketerkaitan) ?>" class="btn <?php echo $dataPanel; ?>"><i class="fa fa-undo"></i> Batalkan</a>
			        </div>
			    </div>
			</div>
		</form>
